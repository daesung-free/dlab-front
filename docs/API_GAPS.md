...
## 12-1. 성적 입력이 앱 전용이라 관리자가 대신 넣을 수 없다

점수 입력 경로가 `/api/v1/app/grades/*` 뿐이고 학생 인증을 요구한다.
관리자는 **시험 양식을 만들고 결과를 조회**할 뿐이다.

설문과 같은 구조라 설계로는 일관된다(관리 = 양식·배포, 앱 = 응답). 다만 운영에서는
**학생이 앱을 안 쓰거나 못 넣는 경우**가 생긴다 — 그때 담임이 대신 넣을 방법이 없다.

> 확인 필요: 대리 입력을 허용할지. 허용한다면 "누가 대신 넣었는지"가 남아야 한다.

## 12-2. 외부 성적 연동·엑셀 업로드가 없다

목업 안내가 이렇게 적고 있다 — *"더프리미엄모의고사 API(전화번호 기반)로 자동 조회되고,
단원별 정답률 등 상세 분석은 엑셀 업로드로 반영된다."*

둘 다 API가 없다. 그래서 **단원별 정답률 박스**를 채울 수 없다.

- 외부 연동은 D-계열 오픈이슈와 엮여 있을 가능성이 크다
- 엑셀 업로드는 학생 일괄 등록(`/students/import`)과 같은 방식이면 된다
