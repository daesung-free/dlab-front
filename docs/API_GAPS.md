...
## 6-4. 특강 신청 기간(`applyFrom`/`applyTo`)이 `Instant` 다
...
- 6-1과 같은 증상이다 — 메시지가 "요청 본문 형식이 올바르지 않습니다"뿐이라 **어느 필드인지 모른다**
