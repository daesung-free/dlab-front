...
#### F-4.1-1 학원생 검색 — `academyId`를 안 받는다

다른 목록은 전부 `academyId`를 받는데 **이 엔드포인트만 안 받는다.** 전 지점 권한 계정에는
항상 전 지점이 한 번에 온다.

- 증상: TopNav에서 '분당'을 골라도 **학생 목록만 전 지점이 그대로 나온다.** 화면 사이에서
  지점 스코프가 어긋나 보인다.
- 지금 대응: 응답의 `academyName`을 컬럼으로 띄워 어느 지점인지 구분만 되게 해뒀다.
- 요청: 다른 목록과 같이 `academyId` 파라미터를 열어줄 것. (없이도 동작은 하므로 우선순위는 낮다)
...
> **2026-09-02 갱신 — 6건 전부 백엔드에서 작업 완료.** (`#66` 시드 · `#67` API, 머지 대기)
> 프론트는 머지 후 아래 "적용 필요" 항목을 반영한다.
>
> **프론트가 반영해야 할 것**
> | 무엇 | 내용 |
> |---|---|
