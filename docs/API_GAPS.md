...
| 수정자 | `actorName` — **아직 `"EMPLOYEE"` 로 온다**(운영 32건 전부). 고쳤다고 들었으나 미배포 |
...
→ 요청 (우선순위 순)
1. `changes` — 이게 없으면 화면이 "누가 뭔가를 건드렸다"까지만 말한다
2. `actorName` 배포
3. 대상 이름·학번 (지금은 `entityId` 숫자뿐이라 사람이 못 읽는다)
4. `action` 필터
5. `academyId` 가 null 인 건이 32건 중 5건 있다 — 지점으로 거르면 그 5건이 샌다
