...
# 22부. 상벌점 목록 정렬 — 같은 날 부여분의 순서가 불안정하다 ★ (F-4.1-2, 2026-09-08)

`GET /penalties` 는 `occurredAt` 내림차순으로 온다. **거기까지는 맞다.**
문제는 **같은 값일 때의 2차 기준이 없다**는 것이다.

## 무엇이 보이나

화면에서 점수를 준 직후 **방금 준 것이 맨 위로 안 온다.** 맨 위 행은 그대로 있고
새 행이 그 아래 아무 데나 끼어든다. 담당자는 "부여가 안 됐나" 하고 한 번 더 누르게 된다.

## 왜

부여 요청은 **날짜만** 보낸다(`occurredAt: "2026-09-08"`). 요구사항이 '발생 일자'이고
**어제 일을 오늘 넣는 경우가 실제로 있어** 날짜만 받는 것이 맞다. 서버는 이를 그날
**자정**으로 저장한다. 그래서 **같은 날 화면에서 준 것이 전부 같은 시각으로 뭉친다.**
...
→ id 36  2026-09-08T08:29:41   ← API 로 직접 부여(시각 있음). 맨 위로 온다
  ...
  id 35  2026-09-08T00:00:00   │ 화면에서 부여한 것들 —
  id 34  2026-09-08T00:00:00   │ 전부 같은 시각이고 이 안의 순서가
  id 33  2026-09-08T00:00:00   │ id 순도 아니다 (28 → 35 → 34 → 33 → 31)
  id 31  2026-09-08T00:00:00   ┘
  id  4  2026-09-07T10:00:00
  …
```

**id 순이 아니다.** 정렬 키가 같을 때 순서가 보장되지 않아, 재조회할 때마다 달라질 수도 있다.

## 요청

> `ORDER BY occurred_at DESC, id DESC` — **2차 정렬 키를 넣어 달라.**
> 같은 날 부여분은 나중에 넣은 것이 위로 와야 한다.

프론트에서 막지 않았다. 화면이 다시 정렬하면 **서버 페이징이 들어오는 날 어긋난다**
(이 엔드포인트는 서버 페이징이 예정돼 있다 — `penalties.ts` 머리 주석). 페이징이 붙으면
정렬은 서버가 하는 것이 맞고, 그때 프론트 정렬은 페이지 안에서만 도는 가짜가 된다.

## 같은 문제가 있을 만한 곳

`occurredAt`·`createdAt` 처럼 **날짜만 받는 목록**은 전부 해당한다. 확인이 필요하다:
사유 신청·출결·상담 이력. **이번에 한 번에 봐 주면 좋겠다.**
