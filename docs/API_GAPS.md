...
- 지금 대응: `patchNotice()` 가 기존 제목·내용을 채워 보낸다
- 위험한 점 — **다른 사람이 그 사이 본문을 고쳤으면 되돌아간다.** 고정 버튼 하나가
  남의 수정을 덮어쓴다. 목록 데이터가 낡았을수록 확률이 올라간다

> 요청: `PATCH` 로 부분 갱신을 받거나, `title`·`content` 를 선택값으로 바꿀 것.
> 고정·배너 토글은 본문과 무관한 조작이다.

## 13-2. 공지 열람 수가 없다

목업의 '열람' 컬럼(281/296 같은 진행률)에 대응하는 값이 응답에 없다.
공지를 몇 명이 봤는지가 발송 화면의 핵심 지표인데 채울 수 없어 `<Unfilled/>` 로 뒀다.

> 앱에서 읽음 처리를 하고 있다면 집계만 실어주면 된다.
...
## 14-1. 슬롯 조회가 날짜 하나씩이라 주간 표를 그리려면 5번 부른다

`GET /qna/offline/slots` 가 `date` 를 필수로 받는다. 목업이 주간 그리드라
월~금 5일치를 `Promise.all` 로 부른다.

루틴 결과(7부)와 같은 패턴이었고 그때는 날짜 기준 일괄 조회가 신설됐다.

> 요청: `from`·`to` 기간 조회. 주 단위 화면이 기본이라 5회가 매번 나간다.

## 14-2. 예약자 응답에 반·과목이 없다

`QnaReservation`: `id, studentId, studentNo, studentName, question, reservedAt, canceledAt`

목업 신청 내역은 **반·과목**을 함께 보여준다. 담당 교사가 "어느 반 누가 무슨 과목을
물어보는지"를 목록에서 훑는 화면이라 둘 다 의미가 있다.

- **반** — 다른 목록(`className`)처럼 실어주면 된다
- **과목** — 예약 자체에 과목 항목이 없다. 신청 폼에 과목 선택이 있는지부터 확인이 필요하다
