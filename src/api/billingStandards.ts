...
/* ── 교습비 단가표 (PRICE_MATRIX 행의 하위 화면) ────────────────
 *
 * 청구기준의 교습비 행은 금액을 한 칸에 못 넣어 여기로 내려온다.
 * 학년 × 좌석유형으로 갈리기 때문이다. */

/** 좌석유형. 1인실이 독서실비가 더 비싸다 */
...
export interface TuitionPrice {
  ...
  tuitionFee: number
  studyRoomFee: number
  /** 서버가 합계를 같이 준다 — 화면이 더하지 않는다 */
  monthlyTotal: number
}

export function listTuitionPrices(params: { year: number; academyId?: number }): Promise<TuitionPrice[]> {
  return request<TuitionPrice[]>('/api/v1/admin/tuition/prices', { query: { ...params } })
}
