...
/**
 * 요구사항 명세 상세 — 내부 개발팀용(`/spec/:screenId`).
 * 제품 화면(`/s/:screenId`)에는 이 내용을 노출하지 않는다.
 */
export function SpecPage() {
  const { screenId } = useParams()
  const s = screenId ? findScreen(screenId) : undefined
  if (!s) return <Navigate to="/spec" replace />
  ...
  return (
    <>
      <PageHead
        ...
        sub={s.summary}
        actions={
          <Link className="btn pri" to={`/s/${s.id}`}>
            <Icon name="external-link" size={14} /> 실제 화면 열기
          </Link>
        }
      />
          ...
          {assumptions.length > 0 && (
            <div className="dsec">
              <h3>
                ...
                잠정 결정 {assumptions.length}건 — 확정되면 화면을 고쳐야 함
              </h3>
              <div className="kv">
                {assumptions.map((a) => (
                  <div className="row" key={a.what} style={{ flexDirection: 'column', gap: 4 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.55 }}>
                      ...
                      {a.issue && (
                        <>
                          {' '}
                          <code style={{ fontSize: 11 }}>{a.issue}</code>
                        </>
                      )}
                    </div>
                  </div>
