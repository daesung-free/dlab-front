/* ============================================================================
 * 화면별 **내부 메모** — 요구사항정의서에서 옮겨온 것 중 제품에 노출되지 않는 항목.
 *
 * ★ `menu.ts` 에서 떼어낸 이유. menu.ts 는 사이드바가 쓰므로 **프로덕션 번들에 반드시
 *   들어간다.** 거기에 DSA 실사 기록·미팅 메모·DB 테이블명이 함께 있으면 화면에 안
 *   그려도 개발자도구에서 그대로 읽힌다. 번들은 로그인 전에 내려간다.
 *
 * ★ 이 파일은 `/spec`(내부 요구사항 뷰)만 import 한다. 그 라우트가 프로덕션에서
 *   stub 으로 바뀌므로 이 파일도 함께 그래프에서 빠진다 — `vite.config.ts` 주석 참고.
 *
 * ⚠ **제품 화면(`src/pages/screens/*`)에서 import 하지 말 것.** 하나라도 import 하면
 *   프로덕션 번들에 다시 들어온다. 배포 전 확인:
 *     npm run build && grep -c "penalty_items\|0723" dist/assets/*.js   → 0
 * ========================================================================== */

export interface ScreenInternal {
  /** 핵심 요구사항 / 로직 (G열) */
  logic?: string[]
  /** 주요 데이터 항목 (H열) */
  tables?: string[]
...
export const SCREEN_INTERNAL: Record<string, ScreenInternal> = {
  'student-search': {
    logic: [
...
export const internalOf = (id: string): ScreenInternal => SCREEN_INTERNAL[id] ?? {}
