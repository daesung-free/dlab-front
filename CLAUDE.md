...
- **자동화 브라우저에서는 즉시 `null` 이다.** 클릭은 전달되는데 요청도 모달도 안 생겨
  **"눌러도 아무 일이 없다"** 로 보인다 — 점검 리포트에 그대로 올라온다(2026-09-13).
- 값을 하나밖에 못 받아 연달아 두 번 묻게 된다. 두 번째를 취소하면 빈 값이 저장된다.
- 대화상자가 떠 있는 동안 **탭 전체가 멈춘다.**

15곳을 걷어냈는데 **새로 만든 화면에서 다시 들어온 적이 있다.** 화면을 새로 만들 때
이 규칙을 모르면 반복된다 — `Modal.tsx` 주석에만 적혀 있어서 안 보였다.

확인: `grep -rn "window.prompt\|window.confirm" src/` → 주석 말고는 0건이어야 한다.
...
**실연동된 화면 32개** — 학원생 검색(F-4.1-1), 교무업무 명단(F-4.9), 반 배정(F-4.1-4),
배정 관리(F-4.10-3), 상담(F-4.11-4), 데일리 루틴(F-4.11-1), 독서실 좌석배치(F-C-4),
주·일 학습 계획(F-4.11-2), 출결 관리(F-4.3), 사유 신청(F-4.1-5), 상벌점(F-4.1-2),
수납현황(F-4.8 — 청구 등록은 교습비(월)·입학(첫 달)·그 밖 셋으로 갈린다), 급식 관리(F-4.5), 특강 관리(F-4.7), 신규 접수 등록(F-4.1-3),
승인 라우팅(F-4.11-5), 설문 관리(F-4.6-부속), 성적 관리(F-4.6), 메시지 관리(F-4.11-3), 질의응답(F-4.11-7),
문자 발송(F-4.4 — 템플릿 관리·발송 이력 탭), 연간 행사(F-4.11-10 — 쉬는 날 + 행사), 앱 운영(F-C-6 — 푸시 발송 탭 제외),
기초 관리(F-4.10-1), 수납 관리(F-4.10-5 — 환불 기준은 읽기 전용), 특강 기초 설정(F-4.10-4),
금일 수정 이력(F-C-1 — 감사 로그. 변경 전후 값은 직원 계정만 온다 — 학생·성적·청구는 서버가 아직 안 준다),
학원생 현황(F-C-2 — 반별 휴원·퇴원·재수 구분은 학생 목록으로 센다),
실적 관리(F-4.10-6 — 지점 전체 목록 + 엑셀 일괄 등록),
성적 업로드(F-4.6 부속 — 0921 성적 문서로 새로 생긴 화면. 디랩 시험 회차 등록 + 파일 3종),
좌석 이탈/복귀(F-4.11-8 — 이탈 이력 + 좌석표에 '이탈 중'. 이탈 위치·키오스크 관리 탭은 서버에 없다).
나머지 5개는 아직 목업이다(아래 두 부류).
...
**목업으로 남은 화면은 두 부류다 — 섞지 않는다(2026-09-22 기준).**

| 부류 | 화면 | 뜻 |
|---|---|---|
| 관리자 API 없음 | 신상기록부(F-4.11-9) · Daily Report 집계(F-4.11-6) · 시간표(F-C-3) | 붙일 엔드포인트를 찾지 말 것 |
| **API 있지만 멈춤** | 대기자 관리(F-4.2) · 결제 관리(F-C-5) | **결정으로 멈춘 것이다.** 풀리면 바로 붙인다 |

- 대기자 관리 — `/admission-reservations`(목록·메모·상태 이력·원생 전환)가 있다. 신청이 우리 쪽으로
  들어오는 경로가 없어 **문서만** 하기로 했다(API_GAPS 26부)
- 결제 관리 — `/payment-requests`(단말기·가상계좌)가 있다. "지금은 말고" 로 보류(API_GAPS 25부)
- `/schedules` 는 시간표가 아니라 **정기일정**이다(이미 붙어 있다) — 이름만 보고 착각하지 말 것

★ **"API 없음" 을 판단하기 전에 `npm run api:types` 부터 돌린다.** `schema.d.ts` 가 9/1 이후 17일
갱신되지 않은 채로 "없다" 고 판단했다가 실적·학원생 현황·입학 예약을 놓칠 뻔했다(2026-09-21).
...
- **"확인 필요"로 적기 전에 `dlab-api/CLAUDE.md` 를 먼저 본다.** 클라이언트 회신으로 이미
  닫힌 사항이 거기 정리돼 있다 — 실제로 성적 입력 주체를 "확인 필요"로 올렸다가
  0826 회신에 답이 있던 것을 뒤늦게 발견했다(API_GAPS 12부).
...
전부 **오류가 안 나고 화면만 이상해져서** 늦게 발견된다. 확인: `npm run check:frow`
(폼 배치 · 안내 상자 배치 · 모달 숫자 입력 · 눌리지 않는 체크박스 · 눌러도 아무 일 없는 버튼 · 메뉴 코드 매핑)
...
### `.note-box` 도 flex 다 — 글자와 `<b>` 를 형제로 두지 않는다

`.frow` 와 같은 함정이다. 안내 상자 안에서 **글자와 태그가 형제면 각각이 칸이 되어**
가운데가 세로로 눌린다. 한 칸에 묶는다.

```tsx
✗ <div className="note-box">반만 복사할 수 없습니다. <b>학과·과정</b>이 함께 넘어갑니다.</div>
✓ <div className="note-box"><div>반만 복사할 수 없습니다. <b>학과·과정</b>이 함께 넘어갑니다.</div></div>
```

글자만 있으면 칸이 하나라 괜찮다. `.ic` + 본문 2칸 배치는 원래 쓰던 모양이다.

### `Modal` 안 숫자 입력에 `step` 을 걸지 않는다

`Modal` 은 내용을 `<form>` 으로 감싸고 확인 버튼이 submit 이다. 값이 `step` 배수가 아니면
**브라우저가 제출을 막는다** — 요청도 안 나가고 우리 오류도 안 뜬다. 네이티브 말풍선이
잠깐 뜰 뿐이라 **"저장 버튼이 안 먹는다"** 로 보인다.

```tsx
✗ <input type="number" min={1} step={100} />   // 7,700 이 막힌다 (기준점이 min 이라 7,701 만 유효)
✓ <input type="number" min={0} />              // 정수면 step 을 안 쓴다
✓ <input type="number" step="any" min={1} max={9} />  // 소수가 필요하면 any
```

범위(`min`·`max`)는 그대로 둔다 — 그건 말풍선으로 이유가 보인다.
...
`onClick` 도 `disabled` 도 없는 버튼은 **고장으로 읽힌다.** 아직 못 붙였으면
`disabled data-soon title="준비 중입니다"` 로 막는다 — 자동화 점검 리포트에도
"눌러도 반응 없음"으로 올라온다.

**값이 없는데 있는 척하지 않는다.** 상단바 알림에 빨간 배지와 '3건'이 하드코딩돼
있었다. 서버에 그 개념 자체가 없는데 읽지 않은 알림이 있는 것처럼 보였다.

### 헤더 액션은 본문과 상태를 공유하지 못한다

`Mockup.actions` 는 `ScreenPage` 가 **본문(`Content`)과 따로 렌더한다.** 같은 화면인데
부모가 달라 props 로 내릴 자리가 없다 — 헤더에서 뭘 만들어도 본문 목록이 그대로 남는다.

`screenSignal.ts` 의 `createScreenSignal()` 을 **화면당 하나씩** 만들어 잇는다.
전역 하나를 나눠 쓰면 남의 화면 갱신까지 끌려온다.

### 목록을 거를 때 서버 집계와 대조한다 ★

화면이 만든 필터가 서버가 세는 기준과 다르면 **오류 없이 숫자만 틀린다.**

- 특강 신청 취소는 행을 지우지 않고 `status` 를 `CANCELED` 로 바꾸는데
  **`waitlisted` 는 false 로 남는다.** `!waitlisted` 로 확정자를 세면 취소자가 섞여
  서버는 4명, 화면은 5명이 됐다(2026-09-16)
- 합계를 낼 때 **코드 목록이 학년·연도마다 다를 수 있다.** 국수탐 백분위합이
  고3·N수 코드(`INQUIRY1/2`)만 갖고 있어 고2는 국어+수학만 더했다

붙이고 나서 **서버 요약 API 와 화면 계산을 한 번 대조한다.** 수납(`/receipt-status/summary`)·
상벌점(`summary`)·특강(`confirmedCount`)이 그 대조 대상이다.
