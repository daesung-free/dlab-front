...
## 1-1. 화면 문구는 사용자 언어로 쓴다

목업에 **개발자용 설명이 화면에 노출된 경우가 있었다**(테이블명·enum·DB 제약). 클라이언트가
읽을 이유가 없고, 데모에서 보이면 "이게 뭐냐"는 질문만 생긴다.

- 화면에는 **사용자가 그 화면에서 판단하는 데 필요한 것만** 쓴다
- 설계 근거(왜 그렇게 만들었는가)는 **파일 상단 주석**에 남긴다 — 지우지 말고 옮긴다
- 판단 기준: *행정 선생님이 이 문장을 읽고 할 일이 달라지는가?* 아니면 주석으로 간다

```
✗ 채번 규칙은 year + %04d(nextSeq)이며 제약은 UNIQUE(year, student_no)입니다
...
**실연동된 화면 20개** — 학원생 검색(F-4.1-1), 교무업무 명단(F-4.9), 반 배정(F-4.1-4),
배정 관리(F-4.10-3), 상담(F-4.11-4), 데일리 루틴(F-4.11-1), 독서실 좌석배치(F-C-4),
주·일 학습 계획(F-4.11-2), 출결 관리(F-4.3), 사유 신청(F-4.1-5), 상벌점(F-4.1-2),
수납현황(F-4.8), 급식 관리(F-4.5), 특강 관리(F-4.7), 신규 접수 등록(F-4.1-3),
승인 라우팅(F-4.11-5), 설문 관리(F-4.6-부속), 성적 관리(F-4.6), 메시지 관리(F-4.11-3), 질의응답(F-4.11-7).
나머지 16개는 아직 목업이다.
화면별 진행 상황은 `docs/CONNECT_PLAN.md`.
...
값이 정말 없는 것(미배정 학생의 좌석)은 `'-'`, 서버가 아직 안 주는 것은 `<Unfilled/>`로 구분한다.

**쓰기(배정 등)는 일괄 API가 없는 경우가 많다.** N번 호출하게 되므로 **건별 성공/실패를 모아
화면에 보여준다.** 중간에 실패하면 일부만 반영된 채로 남는데, 그걸 안 알려주면
사용자는 전부 된 줄 안다.

**저장이 여러 단계로 나뉘면 부분 성공을 반드시 구분해서 알린다.** 예: 신규 접수 등록은
`POST`(등록) → `PATCH`(상세) 두 번 나가는데, 첫 단계만 성공한 것을 "저장 실패"로 뭉뚱그리면
담당자가 다시 등록해 **중복 데이터**가 생긴다. "무엇이 됐고 무엇이 안 됐는지"를 그대로 쓴다.
