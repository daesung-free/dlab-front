...
기초 관리(F-4.10-1), 수납 관리(F-4.10-5 — 환불 기준 탭 제외), 특강 기초 설정(F-4.10-4),
금일 수정 이력(F-C-1 — 감사 로그. 변경 전후 값은 서버가 아직 안 준다),
학원생 현황(F-C-2 — 반별 휴원·퇴원과 재수 구분은 서버가 아직 안 준다),
실적 관리(F-4.10-6 — 전체 목록 API 가 없어 학생을 골라서 본다),
