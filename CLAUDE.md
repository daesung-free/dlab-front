...
**실연동된 화면 31개** — 학원생 검색(F-4.1-1), 교무업무 명단(F-4.9), 반 배정(F-4.1-4),
...
**목업으로 남은 화면은 두 부류다 — 섞지 않는다(2026-09-21 기준).**

| 부류 | 화면 | 뜻 |
|---|---|---|
| 관리자 API 없음 | 신상기록부(F-4.11-9) · Daily Report 집계(F-4.11-6) · 좌석 이탈/복귀(F-4.11-8) · 시간표(F-C-3) | 붙일 엔드포인트를 찾지 말 것 |
