...
**실연동된 화면 29개** — 학원생 검색(F-4.1-1), 교무업무 명단(F-4.9), 반 배정(F-4.1-4),
...
**API가 아예 없어 목업으로 두는 화면 5개** — 붙일 엔드포인트를 찾지 말 것:
대기자 관리(F-4.2) · 신상기록부(F-4.11-9) · 실적 관리(F-4.10-6) ·
Daily Report 집계(F-4.11-6) · 결제 관리(F-C-5)

★ 결제 관리는 **PG 가 아직 안 붙었다**(2026-09-11 확인). `/billings` 는 청구·수납이라
도메인이 다르다 — 그건 이미 수납현황(F-4.8)에 붙어 있다. API_GAPS 24-2 참고.
